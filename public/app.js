...
function pricePrimary(s,p,krw){return Number(krw||p?.price)>0?fmt(krw||p.price):''}
function marketMatch(s,market){if(!market)return true;if(market==='KR')return s.country==='KR';return s.market===market}
...
function stockCard(s){
  ...
  const primary=p?.price?pricePrimary(s,p):blocked?'거래 불가':'클릭해 시세 확인';
  const change=p?.price?`${changeText(p)} · ${Number(p.change||0)>=0?'+':''}${fmt(p.change)}`:'검색 가능한 상장 종목';
  return `<article class="stock-card ${blocked?'stock-blocked':''}" data-code="${esc(s.code)}"><div class="stock-head"><div><div class="stock-name">${esc(s.name)} ${status?`<span class="status-badge">${status}</span>`:''}</div><div class="stock-code">${esc(codeLabel(s))}</div></div><div class="stock-sector">${esc(s.market||'국내')}</div></div><div class="stock-price">${primary}</div><div class="change ${changeClass(p?.changeRate||0)}">${status||change}</div><div class="stock-foot"><span>${esc(sourceText(p))}</span><span>${p?.delayed?'지연 데이터':''}</span></div></article>`
}
...
async function init(){
  ...
  const refreshMinutes=Math.round(Number(config?.marketData?.kr?.refreshMs||config?.marketDataRefreshMs||3_600_000)/60_000),refreshLabel=refreshMinutes===60?'1시간':`${refreshMinutes}분`;
  $('#priceStatus').textContent=`국내 ${Number(config.universe.count||0).toLocaleString()}개 종목 검색 가능 · 공공데이터 ${config?.marketData?.kr?.asOfDate?dateKo(config.marketData.kr.asOfDate):'준비 중'} · ${refreshLabel}마다 새 자료 확인 · 수수료 ${(Number(config.tradeFeeRate||0)*100).toFixed(3)}%`;
  renderMarket();
