...
function renderMarketData(md){const kr=md?.kr||{};$('#marketDataStatus').innerHTML=`<b>국내</b> · ${esc(kr.source||'금융위원회 공공데이터')} · 기준일 ${esc(mdDate(kr.asOfDate))} · ${Number(kr.count||0).toLocaleString()}개 · 확인 주기 ${Math.round(Number(kr.refreshMs||0)/60000).toLocaleString()}분${kr.error?`<br><span class="down">${esc(kr.error)}</span>`:''}`}
