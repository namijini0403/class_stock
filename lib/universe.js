...
function isDomesticStock(s){ return Boolean(s&&s.country==='KR'&&s.currency==='KRW'&&/^\d{6}$/.test(String(s.code||''))); }
...
class StockUniverse{
  ...
  setStocks(stocks,source=this.source,updatedAt=new Date().toISOString(),retiredStocks=[...this.retired.values()]){
    const map=new Map(),retiredCodes=new Set((retiredStocks||[]).filter(isDomesticStock).map(s=>s.code));
    for(const src of stocks||[]){ if(!isDomesticStock(src)) continue; const s={...src,active:src.active!==false}; if(!map.has(s.code)) map.set(s.code,s); }
    for(const f of this.fallback){ if(isDomesticStock(f)&&!map.has(f.code)&&!retiredCodes.has(f.code)) map.set(f.code,{...f,active:true,tradingHalt:false,liquidation:false}); }
    ...
    this.retired=new Map((retiredStocks||[]).filter(s=>isDomesticStock(s)&&!map.has(s.code)).map(s=>[s.code,{...s,active:false}]));
  ...
  replaceMarket(country,newStocks,{source,updatedAt=new Date().toISOString(),trackEvents=true}={}){
    country=String(country).toUpperCase(); const oldActive=new Map(this.stocks.filter(s=>s.country===country).map(s=>[s.code,s])); const next=new Map((newStocks||[]).filter(isDomesticStock).map(s=>[s.code,s])); const events=[];
    if(trackEvents){
      for(const [code,n] of next){ const retired=this.retired.get(code),o=oldActive.get(code)||retired; if(!o) events.push({type:'NEW_LISTING',code,newName:n.name,effectiveDate:updatedAt.slice(0,10),sourceKey:`NEW:${code}:${updatedAt.slice(0,10)}`}); else if(retired) events.push({type:'RESTORED',oldCode:code,newCode:code,oldName:o.name,newName:n.name,effectiveDate:updatedAt.slice(0,10),sourceKey:`RESTORED:${code}:${updatedAt.slice(0,10)}`}); else if(o.name&&n.name&&o.name!==n.name) events.push({type:'RENAME',oldCode:code,newCode:code,oldName:o.name,newName:n.name,effectiveDate:updatedAt.slice(0,10),sourceKey:`RENAME:${code}:${n.name}`}); this.retired.delete(code); }
