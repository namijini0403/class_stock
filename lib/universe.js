...
class StockUniverse{
  ...
  replaceStocks(newStocks,{source,updatedAt=new Date().toISOString(),trackEvents=true}={}){
    const oldActive=new Map(this.stocks.map(s=>[s.code,s])),next=new Map((newStocks||[]).map(normalizeStock).filter(Boolean).map(s=>[s.code,s])),events=[];
    if(trackEvents){
    ...
    this.setStocks([...next.values()],source||this.source,updatedAt,[...this.retired.values()]); this.persist(); return events;
  }
