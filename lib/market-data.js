...
class MarketDataService{
  ...
  loadCaches(){
    try{ if(fs.existsSync(this.krFile)){ const d=JSON.parse(fs.readFileSync(this.krFile,'utf8')); const items=[...new Map((d.items||[]).filter(q=>/^\d{6}$/.test(String(q.code||''))&&(q.country||'KR')==='KR'&&(q.currency||'KRW')==='KRW').map(q=>[q.code,q])).values()]; const expected=this.universe?.stocks?.length||0,minCount=minimumKrCount(expected); if(items.length>=minCount){ for(const q of items)this.kr.set(q.code,q); this.krMeta={...this.krMeta,...d.meta,count:items.length}; const stocks=items.map(q=>({code:q.code,displayCode:q.code,symbol:q.code,name:q.name,market:q.market,country:'KR',currency:'KRW',active:true,tradingHalt:false,liquidation:false,isinCd:q.isinCd||''})); this.universe.replaceMarket('KR',stocks,{source:'FINANCE_PUBLIC_DATA_CACHE',updatedAt:new Date(d.meta?.updatedAt||Date.now()).toISOString(),trackEvents:false}); } else if(items.length){ this.krMeta.error=`국내 캐시 종목 수가 충분하지 않습니다: ${items.length}/${minCount}`; } } }catch(e){this.krMeta.error=`국내 캐시 읽기 실패: ${e.message}`;}
  }
  ...
  async refreshKr(force=false){
    ...
    this.krInflight=(async()=>{ try{
      ...
      const events=this.universe.replaceMarket('KR',stocks,{source:'FINANCE_PUBLIC_DATA',updatedAt:new Date().toISOString()}); this.kr=new Map(items.map(q=>[q.code,q])); this.krMeta=nextMeta; return {...this.status().kr,events};
