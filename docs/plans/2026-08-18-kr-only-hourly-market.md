...
- 다중시장 호환 코드는 `server.js`, `lib/market-data.js`, `lib/universe.js`, `public/app.js`, 종목 캐시와 회귀검사에 분산돼 있다.
...
| 7 | 비국내 호환 계층과 응답 필드 완전 제거 | 서버·시세·종목·학생 상태 보기·문서 | 메인 | 현재 추적 트리 전역 금지 검사와 상태 보기 단위검사 | ☑ |
...
- [x] 비국내 호환 계층 제거와 국내 전용 회귀검사 완료

## 5. 리스크·롤백

- 최신 원격 온라인 저장 기능이 구버전 코드로 되돌아갈 위험 → `origin/main` 기반 별도 통합 브랜치만 푸시하고, 구버전 수정은 로컬 백업 브랜치에 격리한다.
- 기존 비국내 보유 상태를 자동 삭제하면 현금·장부가 불일치할 수 있음 → JSONB 원본은 보존하고 학생 응답·교사 평가·기업행동 적용에서 국내 6자리 코드만 허용한다.
- 공공데이터 장애 → 마지막 정상 캐시를 유지하고 매시간 다시 확인한다.
- 롤백: 이번 호환 계층 제거 커밋 하나를 되돌리면 직전 원격 `6557bb9` 상태로 복귀할 수 있다. 학생 DB 스키마 변경은 하지 않는다.
