# 우리학교 모의투자 v3.1.0 — 국내 주식 전용·일봉 차트
...
- 종목 상세에서 최근 30·90·180일 범위의 일봉 차트 제공
...
국내 데이터는 일별 OHLCV(시가·고가·저가·종가·거래량) 자료입니다. 장중 실시간 또는 시간봉 데이터가 아니며, 공식 자료는 기준일의 **다음 영업일 오후 1시 이후** 반영될 수 있습니다. 서버는 한국시간 14:10에 하루 1회 새 자료를 확인하고 화면에는 실제 기준일을 표시합니다. 일시적 오류는 30분 뒤 제한적으로 재시도하며, 주말·공휴일에는 최신 기준일이 더 이전일 수 있습니다.
...
사용 필드: 기준일, 종목코드/명, 시장구분, 종가, 전일대비, 등락률, 시가/고가/저가, 거래량 등. 차트는 이 공식 일별 OHLCV로 최근 30·90·180일 범위를 표시하며, 휴장일에는 봉을 만들지 않고 시간봉이나 장중 가격으로 재해석하지 않습니다.
...
4. **국내 공공데이터 확인**
   서버는 시작할 때 캐시를 확인하고 공공데이터포털의 새 일별 자료는 한국시간 14:10에 하루 1회 확인합니다. 이 시각 뒤 재배포되어도 당일 확인을 건너뛰지 않으며, 일시적 오류는 30분 뒤 제한적으로 재시도합니다. 공식 자료는 기준일 다음 영업일 오후 1시 이후 반영될 수 있습니다. 장중 실시간 시세나 시간봉이 아니므로 학생 화면의 기준일을 확인하세요.
